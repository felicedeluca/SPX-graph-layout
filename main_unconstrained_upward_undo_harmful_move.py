...
def extract_random_subgraph(G_org, G_directed_org, X, d):
  import random
  edge_list = []
  n = len(G_org.nodes())
  #while len(edge_list)<100:
  if len(G_directed_org.edges())<d:
    quit()
  n_fail = 0
  if len(G_directed_org.edges())<50:
    s=0
    edge_list = [(u, v) for (u, v) in G_directed_org.edges()]
  else:
    while not(d<=len(edge_list)<=50):
      n_fail += 1
      if n_fail >100: quit()
      s = random.randint(0, n-1)
      #print(G_directed_org.edges(), list(G_org.nodes())[s], d)
      edge_list = list(nx.dfs_edges(G_directed_org, source=list(G_org.nodes())[s], depth_limit=d))
  sub_n = 0
  org_to_sub = dict()
  sub_to_org = dict()
  for e in edge_list:
    u, v = e
    if u not in org_to_sub.keys():
      org_to_sub[u] = sub_n
      sub_to_org[sub_n] = u
      sub_n += 1
    if v not in org_to_sub.keys():
      org_to_sub[v] = sub_n
      sub_to_org[sub_n] = v
      sub_n += 1
  edge_list = [(org_to_sub[u], org_to_sub[v]) for (u, v) in edge_list]
  sub_G_directed = nx.DiGraph()
  sub_G_directed.add_edges_from(edge_list)
  sub_G = nx.Graph(sub_G_directed)
  sub_X = np.zeros((sub_n, 2))
  for i in range(len(sub_X)):
    sub_X[i][0] = X[sub_to_org[i]][0]
    sub_X[i][1] = X[sub_to_org[i]][1]
  return sub_G, sub_G_directed, sub_X, org_to_sub, sub_to_org, s

def full_coordinates(sub_n, sub_X, X_old, sub_to_org):
  X_new = np.copy(X_old)
  for i in range(len(sub_X)):
    X_new[sub_to_org[i]][0] = sub_X[i][0]
    X_new[sub_to_org[i]][1] = sub_X[i][1]
  return X_new

def full_direction(sub_D, org_n, sub_to_org):
  full_D = np.zeros((org_n, 2))
  for i in range(len(sub_D)):
    full_D[sub_to_org[i]][0] = sub_D[i][0]
    full_D[sub_to_org[i]][1] = sub_D[i][1]
  return full_D

def subgraph_coordinates(sub_n, X, sub_to_org):
  sub_X = np.zeros((sub_n, 2))
...
#G_d = nx.DiGraph()
#G_d.add_edges_from([(i, i+1) for i in range(100)]+[(99,0)])
##G_d.add_edges_from([(0, 1), (0, 2), (0, 3)])
##G_d.add_edges_from([(1, 0), (2, 0), (3, 0)])
#G = nx.Graph(G_d)
#X = np.random.rand(len(G.nodes()), 2)
#s_G, s_G_d, sub_X, org_to_sub, sub_to_org, s = extract_random_subgraph(G, G_d, X, 2)
#sub_n = len(sub_X)
#X = full_coordinates(sub_n, sub_X, X, sub_to_org)
#X = full_direction(sub_X, len(X), sub_to_org)


def runOptimizer(G, W, K, wi, ki, dUpwardness_dxArr, width, height, G_directed):
	...
	if USE_INITIAL != '0':
		...
		if USE_INITIAL[len(USE_INITIAL)-4:len(USE_INITIAL)]=='.txt':
			...
			node_coords, edge_list = read_dot_file(OUTPUT_FOLDER+'/'+USE_INITIAL)
	...
	#big to small
	G_org = G
	G_directed_org = G_directed
	X_org = np.copy(X_curr)
	G, G_directed, X_curr, org_to_sub, sub_to_org, src = extract_random_subgraph(G_org, G_directed_org, X_org, 4)
	#G, G_directed, X_curr, org_to_sub, sub_to_org, src = extract_random_subgraph(G_org, G_directed_org, X_org, 2)
	n = G.number_of_nodes()
	m = G.number_of_edges()
	edge_list = [[u, v] for (u, v) in G.edges()]
	dUpwardness_dxArr = np.zeros((G.number_of_nodes(),2))
	for e in G_directed.edges():
		u, v = e
		dUpwardness_dxArr[u,1] += 1
		dUpwardness_dxArr[v,1] -= 1
	...
	return optimize(X_curr, wi, ki, W, K, G, dUpwardness_dxArr, width, height, G_directed, G_org, G_directed_org, X_org, org_to_sub, sub_to_org)
	# plotGraphandStats(X_curr)
...
def optimize(X_curr, wi, ki, W_param, K_param, G, dUpwardness_dxArr, width, height, G_directed, G_org, G_directed_org, X_org, org_to_sub, sub_to_org):
	...
	global total_u_gamma_time, total_gradient_descent_time, total_penalties_after_grad_desc_time, W, K, X_new, NUMBER_OF_CROSSINGS, TIME_AT_BEGINNING, CONTEST_DURATION, n, m, penalties, edge_list, u_params, gammas
	W = W_param
	K = K_param
	...
	while 1: 
		...
		if(THIS_GD_OPTION == 'VANILLA'):
			res = minimize_with_gd(X, dUpwardness_dxArr, width, height, G_directed, G, G_directed_org, G_org, X_org, sub_to_org, num_iters=THIS_NUM_ITERS, alpha=THIS_ALPHA)
		elif(THIS_GD_OPTION == 'MOMENTUM'):
			res = minimize_with_momentum(X, dUpwardness_dxArr, width, height, G_directed, G, G_directed_org, G_org, X_org, sub_to_org, num_iters=THIS_NUM_ITERS, alpha=THIS_ALPHA)
		elif(THIS_GD_OPTION == 'NESTEROV'):
			res = minimize_with_nesterov_momentum(X, dUpwardness_dxArr, width, height, G_directed, G, G_directed_org, G_org, X_org, sub_to_org, num_iters=THIS_NUM_ITERS, alpha=THIS_ALPHA)
		elif(THIS_GD_OPTION == 'ADAGRAD'):
			res = minimize_with_adagrad(X, dUpwardness_dxArr, width, height, G_directed, G, G_directed_org, G_org, X_org, sub_to_org, num_iters=THIS_NUM_ITERS, alpha=THIS_ALPHA)
		elif(THIS_GD_OPTION == 'RMSPROP'):
			res = minimize_with_rmsprop(X, dUpwardness_dxArr, width, height, G_directed, G, G_directed_org, G_org, X_org, sub_to_org, num_iters=THIS_NUM_ITERS, alpha=THIS_ALPHA)
		elif(THIS_GD_OPTION == 'ADAM'):
			res = minimize_with_adam(X, dUpwardness_dxArr, width, height, G_directed, G, G_directed_org, G_org, X_org, sub_to_org, num_iters=THIS_NUM_ITERS, alpha=THIS_ALPHA)
		...
		# small to big
		X_curr = full_coordinates(len(X_curr), X_curr, X_org, sub_to_org)
		G = G_org
		G_directed = G_directed_org
		n = G.number_of_nodes()
		m = G.number_of_edges()
		edge_list = [[u, v] for (u, v) in G.edges()]
		...
		# big to small, initialization for stress and penalty ??? always update n and m
		G_org = G
		G_directed_org = G_directed
		X_org = np.copy(X_curr)
		G, G_directed, X_curr, org_to_sub, sub_to_org, src = extract_random_subgraph(G_org, G_directed_org, X_org, 4)
		...
		if (not USE_NUM_ITERS):
			...
			#if(num_iters >= NUM_ITERATIONS or (time.time()-TIME_AT_BEGINNING) > CONTEST_DURATION):
				return X_curr
...
def jacobian_Mod_Cost(X, dUpwardness_dxArr):
	#Reshape the 1D array to a n*2 matrix
	global weights
	global distances
	global n
	...
	dmodCost_dx = dmodCost_dx + U*dUpwardness_dxArr
...
def safe_update(X, D, G, G_directed, width, height, X_org, sub_to_org):
  X = X.reshape((n, 2))
  D = D.reshape((n, 2))

  X = full_coordinates(len(X), X, X_org, sub_to_org)
  D = full_direction(D, len(X), sub_to_org)
  # for all x in X
  for i in range(len(X)):
  # let v be the vertices corresponding to x
  # find all neighbors u of v
  # count number of intersection from adjacent edges corresponding to x and x+d
  # if the intersection does not increase, still upward and in the grid update x to x+d
    x, y = X[i][0], X[i][1]
    nx, ny = round(X[i][0]+D[i][0]), round(X[i][1]+D[i][1])
    if not (0<=nx<=width and 0<=ny<=height):
      continue
    upward = True
    for u in G_directed[i].keys():
      if ny>=X[u][1]:
        upward = False
        break
    if not upward:
      continue
    for u in G_directed.predecessors(i):
      if ny<=X[u][1]:
        upward = False
        break
    if not upward:
    ...
    for u in G[i].keys():
      for e in G.edges():
        p, q = e
        if (i==p) or (i==q) or (u==p) or (u==q): continue
        if(doIntersect(x, y, X[u][0], X[u][1], X[p][0], X[p][1], X[q][0], X[q][1])):
          count_before += 1
        if(doIntersect(nx, ny, X[u][0], X[u][1], X[p][0], X[p][1], X[q][0], X[q][1])):
          count_after += 1
    if count_after>count_before:
      continue
    X[i][0] = nx
    X[i][1] = ny
  X = subgraph_coordinates(len(sub_to_org.keys()), X, sub_to_org)
  return X.flatten()

def minimize_with_gd(X, dUpwardness_dxArr, width, height, G_directed, G, G_directed_org, G_org, X_org, sub_to_org, num_iters=100, beta1 = 0.9, beta2 = 0.999, alpha = 1e-3, eps = 1e-8):
        for t in range(1, num_iters+1):
                grad = jacobian_Mod_Cost(X, dUpwardness_dxArr)
                #X -= alpha * grad
                X = safe_update(X, -alpha * grad, G_org, G_directed_org, width, height, X_org, sub_to_org)
        return X

def minimize_with_momentum(X, dUpwardness_dxArr, width, height, G_directed, G, G_directed_org, G_org, X_org, sub_to_org, num_iters=100, gamma = 0.9, alpha = 1e-3, eps = 1e-8):
        V = np.zeros(X.shape)
        ...
        for t in range(1, num_iters+1):
                ...
                #X -= V
                X = safe_update(X, -V, G_org, G_directed_org, width, height, X_org, sub_to_org)
        return X

def minimize_with_nesterov_momentum(X, dUpwardness_dxArr, width, height, G_directed, G, G_directed_org, G_org, X_org, sub_to_org, num_iters=100, gamma = 0.9, alpha = 1e-3, eps = 1e-8):
        V = np.zeros(X.shape)

        for t in range(1, num_iters+1):
                grad_ahead = jacobian_Mod_Cost(X + gamma*V, dUpwardness_dxArr)
...
def minimize_with_adagrad(X, dUpwardness_dxArr, width, height, G_directed, G, G_directed_org, G_org, X_org, sub_to_org, num_iters=100, beta1 = 0.9, beta2 = 0.999, alpha = 1e-3, eps = 1e-8):
        R = np.zeros(X.shape)
        ...
        for t in range(1, num_iters+1):
                ...
                X = safe_update(X, -alpha * grad / (np.sqrt(R) + eps), G_org, G_directed_org, width, height, X_org, sub_to_org)

        return X

def minimize_with_rmsprop(X, dUpwardness_dxArr, width, height, G_directed, G, G_directed_org, G_org, X_org, sub_to_org, num_iters=100, gamma = 0.9, beta2 = 0.999, alpha = 1e-3, eps = 1e-8):
        R = np.zeros(X.shape)
...
def minimize_with_adam(X, dUpwardness_dxArr, width, height, G_directed, G, G_directed_org, G_org, X_org, sub_to_org, num_iters=100, beta1 = 0.9, beta2 = 0.999, alpha = 1e-3, eps = 1e-8):
        M = np.zeros(X.shape)
        R = np.zeros(X.shape)
        ...
        for t in range(1, num_iters+1):
                ...
                X = safe_update(X, -alpha * m_hat / (np.sqrt(r_hat) + eps), G_org, G_directed_org, width, height, X_org, sub_to_org)
...
def optimizer_jacob(output_folder, filename, W_start, W_end, K_start, K_end, layout_file_name, num_iterations, norm, OUTPUT_FILE_EXT, THIS_GD_OPTION_param, THIS_ALPHA_param, THIS_NUM_ITERS_param):
	...
	FILENAME = filename
	G, width, height = build_networkx_graph_grid_size(OUTPUT_FOLDER+'/'+FILENAME+'.txt')
	G_directed = build_directed_networkx_graph(OUTPUT_FOLDER+'/'+FILENAME+'.txt')
	dUpwardness_dxArr = np.zeros((G.number_of_nodes(),2))
	for e in G_directed.edges():
		u, v = e
		dUpwardness_dxArr[u,1] += 1
	...
	for W in range(W_start, W_end):
		...
		for K in [math.pow(2,i) for i in range(K_start,K_end)]:
			...
			resultX = runOptimizer(G, W, K, wi, ki, dUpwardness_dxArr, width, height, G_directed)
